...
const Wrapper = styled.div`
  ...
  .action-icon {
    ...
    width: 1.75rem; /* Reduced size of the icon container */
    height: 1.75rem; /* Reduced size of the icon container */
...
const AdminWithdrawal = () => {
  ...
  const handleConfirm = async () => {
    if (clickedButton === "reject") {
      try {
        ...
        await customFetch.post(
          `/withdrawal/admin/reject-withdrawal/${row._id}`
        );
    ...
    if (clickedButton === "complete") {
      try {
        setLoading(true);
        await customFetch.post(
          `/withdrawal/admin/confirm-withdrawal-single/${row._id}`
        );
  ...
  const { refetch: withdrawalList, isLoading, isError } = useQuery(
    ["withdrawals", currentPage, rowsPerPage],
    async () => {
      const { data } = await customFetch.get(
        `/withdrawal/admin/get-withdraw-list`
      );
      return data;
    },
    ...
    <Wrapper>
      ...
      <div className="bg-white p-4 mt-4 rounded-lg">
        ...
        <p className="mb-4 text-gray-700">
          View your users withdrawal history. For any issues, use our live chat
          ...
          <table className="w-full border-collapse text-sm border-t border-b border-gray-200">
            <thead>
              <tr>
                ...
                <th className="border border-gray-200 p-2 bg-gray-100">Fees</th>
                <th className="border border-gray-200 p-2 bg-gray-100">
                  Gateway
                </th>
                ...
                <tr key={row.id}>
...

                  {
                    row.status === "Pending" ? (
                      ...
                      <div
                        className={`action-icon  mr-2 cursor-pointer ${
                          row.status === "Rejected" || row.status === "Completed"
                      ...
                      <td className="border border-gray-200 p-2 flex justify-center">
                      <div
                        className={`action-icon hover:text-gray-700 hover:scale-110 text-gray-400`}
                        onClick={() => {
      ...
      <ModalBox
        ...
        title={clickedButton === "reject" ? "Reject Withdrawal" :clickedButton === "complete" ? "Confirm Withdrawal" : null}
        buttonText={clickedButton === "reject" ? "Reject" :clickedButton === "complete" ? "Confirm" : null}
        message={
