...
let connectedUsersNew = new Set();
 ...
 const supportController = async (socket, io, supportNamespace) => {
    ...
    const fetchTickets = async () => {
      try {
        ...
        const adminSocket = Array.from(supportNamespace.sockets.values()).find(
          (s) => s.user?.role === "admin"
        );
        if (adminSocket) {
    ...
    const fetchTicketMessageData = async (ticketId) => {
      ...
      const userSocket = Array.from(supportNamespace.sockets.values()).find(
        (s) =>
